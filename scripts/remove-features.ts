...
const replaceToggleFunction = (node: Node) => {
  const objectOptions =
    node.getFirstDescendantByKind(SyntaxKind.ObjectLiteralExpression)
  ...
  const onFunction =
    onProperty?.getFirstDescendantByKind(SyntaxKind.ArrowFunction)
  const offFunction =
    offProperty?.getFirstDescendantByKind(SyntaxKind.ArrowFunction)
...
files.forEach(sourceFile => {
  sourceFile.forEachDescendant(node => {
    ...
    if (node.isKind(SyntaxKind.CallExpression) && isToggleComponent(node)) {
